...
def create_sb_event(req: func.HttpRequest) -> func.HttpResponse:
 try:   
    ...
    # Check if casename is provided
    logging.info(f'message value is : {json_data} and quename value is {quename} ')
    if not json_data:
...
def v1(req: func.HttpRequest) -> func.HttpResponse:
    ...
    if name:
        ...
        return func.HttpResponse(
             "This HTTP triggered function executed successfully. Passs a name in the query string or in the request body for a personalized response.",
             status_code=200
        )

@app.route(route="maApiservices", auth_level=func.AuthLevel.ANONYMOUS)
def maApiservices(req: func.HttpRequest) -> func.HttpResponse:
    logging.info('Python HTTP trigger function processed a request.')
